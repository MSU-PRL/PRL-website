...
The **PRL-about-page-timeline-jQuery-compiled.scss** file includes all the package's core CSS files in one location. The **prlCustomCSS-2018.scss** file (see CSS folder section) is setup to automatically incorporate the contents of this file for upload to Mura. Do not make changes to the  file. The timeline is constantly updated by the developer, and as core files are overwritten, we lose any custom changes within those files on update. ***Any CSS changes to the timeline should be made separately in the prlCustomCSS-2018.scss file, under the 'About' section**. 
